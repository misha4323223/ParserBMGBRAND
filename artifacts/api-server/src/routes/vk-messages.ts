...
- Плюс партнёр может разместить виджет с кнопкой «Купить» прямо на своём сайте или соцсетях — покупатели не уходят далеко
...
=== ЗАДАЧА ===
${userRequest?.trim() || "Написать первое сообщение с предложением коллаборации — кратко и по делу"}

Напиши короткое живое сообщение (2-4 предложения). Без официоза, как живой человек.
Заканчивай вопросом — интересно ли им это предложение? Если да — скажи что сами поможем со всем и всё расскажем. Ссылки на регистрацию НЕ давать.
Только текст — без кавычек, без подписи, без пояснений.`;
